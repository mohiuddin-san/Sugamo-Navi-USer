...
export async function getTikTokVideos(): Promise<Video[]> {
  try {
    ...
    console.log('Environment variables:', {
      VITE_APIFY_TOKEN: process.env.VITE_APIFY_TOKEN ? '[REDACTED]' : undefined,
      VITE_TIKTOK_USERNAME: process.env.VITE_TIKTOK_USERNAME,
    });
    ...
    try {
      ...
      console.log('No TikTok cache file or invalid cache, fetching new data:', err);
    }
    ...
    if (shouldFetch) {
      ...
      if (!response.ok) {
        throw new Error(`Apify API error: ${await response.text()}`);
      }

      const run = await response.json();
      console.log('Apify run details:', JSON.stringify(run, null, 2));

      const datasetResponse = await fetch(`https://api.apify.com/v2/datasets/${run.data.defaultDatasetId}/items`);
      if (!datasetResponse.ok) {
        throw new Error(`Apify dataset error: ${await datasetResponse.text()}`);
      }
      ...
      videos = items
        ...
        .filter((video: Video) => video.id && video.videoMeta.coverUrl);
